...
const Legend = () => {
  ...
  const vals = Object.keys(LegendType).filter(key => isNaN(Number(LegendType[key])));
  const texts = Object.keys(LegendType).filter(key => !isNaN(Number(LegendType[key])));
  return (
    <>
    {vals.map((val, index) => <Inline key={texts[index]}><StyledCell border onClick={() => setClickValue(Number(val))} background={legendMap.get(Number(val))} /><p>{texts[index]}</p></Inline>)}
    </>
  )
